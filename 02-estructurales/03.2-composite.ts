...
class MenuCategory implements MenuComponent {
  ...
  //? Sobrecarga de operadores - Item puede ser MenuComponent o un arreglo de MenuComponent. Fijate que tiene nombre, asinto envidias
  ...
  showDetails(indent: string = ""): void {
    console.log(`%c${indent}+ ${this.name}`, COLORS.orange);
    /* this.items.forEach((item) =>
      item instanceof MenuCategory ? item.showDetails("  ") : item.showDetails("    "),
    ); */
    this.items.forEach((item) => item.showDetails(indent + " ")); // de esta forma también añadimos recursivamente un espacio por nivel
  }
}
...
function main() {
  ...
  const coldAppetizers = new MenuCategory("Frias")
  const hotAppetizers = new MenuCategory("Calientes")
  coldAppetizers.add(salad);
  hotAppetizers.add(soup);
  appetizers.add([coldAppetizers, hotAppetizers])
  ...
  const mainMenu = new MenuCategory("Menu Principal");
