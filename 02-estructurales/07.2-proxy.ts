...
class DocumentProxy implements Document {
  ...
  private mustHaveRoles: User["role"][];

  // TODO: Implementar el constructor de la clase DocumentProxy
  constructor(document: ConfidentialDocument, allowingRoles: User["role"][] = []) {
    this.document = document;
    this.mustHaveRoles = allowingRoles;
  }
  displayContent(user: User): void {
    if (this.mustHaveRoles.includes(user.getRole())) {
      // TODO: Implementar la lógica para verificar si el usuario tiene permisos
      // Sólo si es admin puede ver el contenido(al final se pasa el rol por constructor)
      return this.document.displayContent();
    }
