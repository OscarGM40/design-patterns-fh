...
// 2. Clase Facade - ComputerFacade(fijate de nuevo en la composición y posterior delegación que habrá, y como la interfaz Facade expone solo las funcionalidades core y ofusca las operaciones no importantes para el cliente de esta interfaz)
interface ComputerFacadeDependencies {
  readonly cpu: CPU; // fijate que una interfaz define siempre un contrato público, ya que es trabajo de las implementaciones de esa interfaz definir el scope.por ello no existen modificadores de acceso en interfaces.Obviamente, en los types es igual
...
class ComputerFacade {
  ...
  private readonly hardDrive: HardDrive = new HardDrive();
...
function main() {
  //recuerda que el Facade suele inicializar objetos.También no es necesario que use inyección en el constructor, pues Facade siempre va a necesitar esas instancias del subsistema. Un mejor approach sería usar solo Composición sin la inyección.La inyección solo es útil cuando las instancias a pasar sean dinámicas, aqui van a ser siempre las mismas
